...
### 📝 Notion 작성 팁
노션에 새 페이지를 만드신 후, 본문 영역에 위 마크다운 텍스트를 그대로 복사(`Ctrl + C`)해서 붙여넣기(`Ctrl + V`) 하시면 **제목, 인용구, 코드 블록, 토글 메뉴가 노션 스타일에 맞춰 깔끔하게 자동 변환** 됩니다. 대소문자와 주석 규칙을 정석대로 매칭해 두었으니 개발 백서로 유용하게 참고하세요.
