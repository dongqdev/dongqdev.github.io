...
# SAP RAP 실무 표준 개발 프로세스 (테이블 선 빌드형)
**핵심 요약: ** 완전히 새로운 서비스를 만들 때도 **물리 테이블을 아밥 코드로 먼저 잽싸게 생성한 뒤, **`OData UI 서비스`** 마법사로머리-꼬리 껍데기들을 자동 생성하는 것** 이 실무에서 가장 빠르고 휴먼 에러가 없는 200% 정석 루트입니다.

## 📅 STEP 1. 물리 데이터베이스 테이블 직접 생성 (`TABL`)
이클립스에서 마법사 창을 키고 필드를 하나하나 마우스로 추가하는 것은 비효율적입니다. 메모장이나 아밥 에디터에서 DDL 코드로 1분 만에 테이블을 먼저 구워버립니다.
...
3. 생성된 12개 파일의 체크박스를 전부 켜고 **한 방에 전체 Activate** 를 진행합니다.
4. 최상단 비즈니스 서비스 계층에 있는 **`Service Binding`** 파일 **을 더블클릭하여 오픈합니다.
5. 엔티티 명을 우클릭하고 **`Preview`** 버튼을 누르면, **OData V4 Draft 임시저장 기능이 완벽 탑재되어**`[생성(Create)]`** 과 **`[삭제(Delete)]`** 버튼이 모두 살아있는 완전체 대시보드 화면** 이 즉시 구동됩니다!
