...
### 3. Step 3-step 7(한번에 생성) {toggle="true"}
> 테이블 생성 후, 패키지 마우스 우클릭 후, Generate ABAP Repository Object

### 3. 데이터 정의 (CDS 인터페이스 뷰) 생성(Data Definition)
>
...
1. efineViewEntity  : 가장 표준적이고 성능이 최적화된 신형 데이터 뷰 엔티티를 만드는 템플릿
...
### ZP_?의 경우, 아래의 draft 관련 소스 추가(Create, Update시 필수)
```javascript
...
```
### 오류조치1 : In CDS, the provider contract "transactional_query" should be defined for the entity "ZP_WBS_TASK”
>

1. Ctrl + 1
2. Add Implementation class to behavior definition 클릭
3. **소스상단에 “implementation** **in** **class** zbp_p_wbs_task **unique;” 생김**
### 오류조치2: class “ZBP_P_WBS_TASK” does not exist
>
...
```
1. ABAP 클래스 생성(NEW > ABAP CLASS)한 후, interfaces ADD > IF_OO_ADT_CLASSRUN<br>* Active 후, F9(실행)
```javascript
...
  ...
  as select from ztwbs_yn_master
{
  ...
  -- 화면단 OData 파이프라인에 '예(Y)', '아니오(N)' 형태로 포맷을 완성해 전달합니다.
  -- concat( concat( yn_text, '(' ), concat( yn_code, ')' ) ) as DisplayText
}

```
1. ZC_TWBS_TASK2 인터페이스 뷰에 콤보박스 설정
```javascript
@Consumption.valueHelpDefinition: [{
  entity: { name: 'ZC_TWBS_CATEGORY', element: 'CategoryName' },
...
1. “10”에서 만든 ZIAM_WBS_CHAT_EXT 마우스 우클릭 후, service 선택
1. Service Type : OData V4
2. Service Name : ZUI_WBS_TASK_O4
2. Save > Active(Ctrl + F3)
3. Business Catalog에서 우측 상단 Publidh Locally 클릭
<columns>
<column ratio="62.5">
...
### ※ 아키텍쳐 방향성 및 BE,FE간 명확한 역할(Role Separation) 지정
> ### 백엔드 레이어 (SAP RAP)
...
1. 프론트 Deploy 네이밍
1. YY1_Z+회사명+모듈+0010
2. YY1_ZUMRMM0010, YY1_ZJUGSMM0010, YY1_ZWDXMM0010, YY1_ZITMSD0010
3. **Deployment Description** : 설명
4. **Select How You Want to Enter the Package**: Choose from Exsting
5. **Package**: 배포하려는 패키지
<columns>
<column ratio="50">
>

1. **Semantic Object**: URL에 노출되는 이름
2. **Action(UI스타일**): diaplay(조회-단순 List Report), analyze(분석)
3. **Title**: 상단 제목
4. **Subtitle**: 부제목(*사용하지 않음)
</column>
<column ratio="50">
...
- url: [https://myXXXXXX](https://myXXXXXX/)… > url: [https://myXXXXXX](https://myXXXXXX/)…
